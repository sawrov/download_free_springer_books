...
cat links_to_books.txt| sort -u | while read line; do { wget -qO- $line |  grep "Download this book in PDF format" | grep -o -P '(?<=href=").*(title)' | grep -o '^\S*' | sort -u | tee -a  pdf_download_links.txt; echo "$count of $final_number ";count=$(( count+1 )); };done
